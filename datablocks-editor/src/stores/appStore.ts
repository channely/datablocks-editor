  ...
  persist(
    immer((set, get) => ({
      ...
      updateConnection: (connectionId: string, updates: Partial<Connection>) => {
        set((state) => {
          const connIndex = state.connections.findIndex(conn => conn.id === connectionId);
          if (connIndex !== -1) {
      ...
      removeError: (errorId: string) => {
        set((state) => {
          state.errors = state.errors.filter(error => 
            `${error.timestamp.getTime()}-${error.type}` !== errorId
          );
        });
      },
      ...
      getSelectedConnections: () => {
        const state = get();
        return state.connections.filter(conn => state.selectedConnections.includes(conn.id));
      },
    })),
    {
